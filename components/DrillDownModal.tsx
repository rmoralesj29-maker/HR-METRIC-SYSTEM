    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[100] p-4 animate-fade-in">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[80vh] flex flex-col animate-fade-in-up">
          ...
          <table className="w-full text-left text-sm whitespace-nowrap">
            <thead className="bg-slate-50 text-slate-600 font-medium">
              <tr>
                ...
                <th className="px-4 py-3">VR Status</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {employees.map((emp) => (
                <tr key={emp.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3 text-slate-800">{emp.totalExperienceMonths} mo</td>
                  <td className="px-4 py-3 text-slate-500">{emp.age}</td>
                  <td className="px-4 py-3 text-slate-500">{emp.country}</td>
                </tr>
              ))}
              {employees.length === 0 && (
                  <tr><td colSpan={6} className="text-center py-4 text-slate-400">No employees found.</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        <div className="p-6 border-t border-slate-100 flex justify-end">
             <button onClick={onClose} className="px-4 py-2 bg-slate-100 text-slate-600 font-medium rounded-lg hover:bg-slate-200 transition-colors">Close</button>
        </div>
      </div>
    </div>
  );
