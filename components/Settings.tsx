...
export const Settings: React.FC<SettingsProps> = ({
  ...
  const [pendingSickDays, setPendingSickDays] = useState(settings.monthlySickDays);
  const [hasUnsavedSickDays, setHasUnsavedSickDays] = useState(false);

  useEffect(() => {
    setPendingSickDays(settings.monthlySickDays);
    setHasUnsavedSickDays(false);
  ...
  const handleSickDayChange = (index: number, value: number) => {
    const newSickDays = [...pendingSickDays];
    newSickDays[index] = { ...newSickDays[index], value };
    setPendingSickDays(newSickDays);
    setHasUnsavedSickDays(true);
  };

  const handleSaveSickDays = () => {
    onUpdateSettings({ ...settings, monthlySickDays: pendingSickDays });
    setHasUnsavedSickDays(false);
  };
          ...
          <div className="max-w-3xl">
            ...
            <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm">
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                {pendingSickDays.map((data, index) => (
              ...
              <div className="flex items-center justify-between px-4 py-3 border-t border-slate-100 bg-slate-50">
                <p className="text-xs text-slate-500">Changes are staged until you click save.</p>
                <button
                  onClick={handleSaveSickDays}
                  disabled={!hasUnsavedSickDays}
                  className="bg-indigo-600 text-white px-4 py-2 rounded-lg font-medium text-sm shadow-sm hover:bg-indigo-700 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  Save sick days
                </button>
              </div>
