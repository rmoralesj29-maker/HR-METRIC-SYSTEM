...
import { SystemSettings, ColumnDefinition, Employee, CalculatedEmployeeStats } from '../types';
import { Save, Plus, Trash2, Download, Table, Settings as SettingsIcon, Layout, Thermometer } from 'lucide-react';
...
export const Settings: React.FC<SettingsProps> = ({ 
  ...
  const handleExport = () => {
    const headers = [
      'ID', 'Name', 'Role', 'Country', 'Languages', 'DOB', 'Start Date', 
      'Gender', 'Prev Exp (Mo)', 'Age', 'Total Experience (Mo)', 'VR Rate', 
      'In Raise Window', 'Next Milestone (Mo)',
      ...customColumns.filter(c => !c.isSystem).map(c => c.label)
    ];

    const rows = employees.map(e => [
      e.id, e.name, e.role, e.country, e.languages.join(', '), e.dob, e.startDate,
      e.gender, e.previousExperienceMonths, e.age, e.totalMonthsExperience, e.vrRate,
      e.inRaiseWindow ? 'Yes' : 'No', e.monthsUntilNextRate || 'Max',
      ...customColumns.filter(c => !c.isSystem).map(c => e.customData?.[c.id] || '')
    ]);
          ...
          <div className="max-w-2xl">
            ...
            <div className="bg-slate-50 p-8 rounded-xl border border-slate-200 text-center">
              ...
              <button 
                ...
                className="bg-indigo-600 text-white px-8 py-3 rounded-xl font-bold hover:bg-indigo-700 transition-colors shadow-lg shadow-indigo-200 flex items-center gap-2 mx-auto"
              >
